...
export type Visit = { date: string; note?: string; photos?: string[] };
export type Station = { id: string; name: string; types: ("S"|"U"|"R")[]; lines?: string[]; visits: Visit[] };
...
export const stationLabel = (st: Station): string => {
...
export const pickThreeUnvisited = (stations: Station[]): string[] => { const unvisited=stations.filter(s=>(s.visits?.length||0)===0); if(!unvisited.length) return []; const pool=[...unvisited]; for(let i=pool.length-1;i>0;i--){ const j=Math.floor(Math.random()*(i+1)); [pool[i],pool[j]]=[pool[j],pool[i]];} return pool.slice(0,Math.min(3,pool.length)).map(s=>s.id); };
...
const makeSeed = (): Station[] => (seedStations || []).map(s => ({
  id: s.id || uid(),
...
const normalizeStations = (data: Station[]): Station[] => {
